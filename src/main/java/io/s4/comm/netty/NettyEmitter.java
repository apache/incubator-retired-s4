...
public class NettyEmitter implements Emitter, ChannelFutureListener,
    ...
    public NettyEmitter(Topology topology) {
        ...
        Runnable r = new Runnable() {
            public void run() {
                for (ClusterNode clusterNode : NettyEmitter.this.topology
                        .getTopology().getNodes()) {
                    ...
                    while (true) {
                        ChannelFuture f = bootstrap
                                .connect(new InetSocketAddress(clusterNode
                                        .getMachineName(), clusterNode
                                        .getPort()));
                ...
                logger.info("Sender is connected to all " + channels.length
                        + " receivers");
            }
        };
        (new Thread(r)).start();
